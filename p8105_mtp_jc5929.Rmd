...
Next, I tried to format the weight into a pounds. I have separated the columns raisin, simone, and gagne by whitespace, 
...
Below step is to create another data frame containing notes and the date, and save the two data frames into csv files.
**Creating data frame with notes**
```{r}
...
information for a single dog on a specific date. The columns includes variables of the date, dog name, weight, and the weight of a standard object. There are 356 unique dates included. Below is a table showing the number of observations for each dog, along with their average weight and standard deviation.
```{r}
dog_data%>%
  group_by(dog_name) %>%
  summarize(obs=n(),
...
```
Plot on the left shows the density of weight for each dog, we can find that Raisin has much higher weight than Gagne and Simon. From the plot,we can also see that there exits an outlier for Raisin that has a weight of less than 10 pounds. 
